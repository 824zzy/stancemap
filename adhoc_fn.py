...
def clean_us_stance():
    ...
    for i, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if not all(
            [keyword.lower() in row["Tweet"].lower() for keyword in keywords.split()]
        ):
...
def add_state_geo():
    ...
    print(
        df.columns
    )  # Index(['Unnamed: 0', 'City', 'Claim', 'Tweet', 'Latitude', 'Longitude', 'User', 'Timestamp', 'Stance', 'Category'],
    ...
    cities = {
        ...
        "Bridgeport": (
            "Bridgeport, Greater Bridgeport Planning Region, Connecticut, United States",
            41.1792695,
            -73.1887863,
        ),
        "Rochester": (
        ...
        "Minot": (
            "Minot, Montbard, Côte-d'Or, Bourgogne-Franche-Comté, France métropolitaine, 21510, France",
            47.6698594,
            4.8786165,
        ),
        "Fargo": (
        ...
        "Olympia": (
            "Ολυμπία, ΕΟ74, Αρχαία Ολυμπία, Δημοτική Ενότητα Αρχαίας Ολυμπίας, Δήμος Αρχαίας Ολυμπίας, Περιφερειακή Ενότητα Ηλείας, Περιφέρεια Δυτικής Ελλάδας, Αποκεντρωμένη Διοίκηση Πελοποννήσου, Δυτικής Ελλάδας και Ιονίου, 270 65, Ελλάς",
            37.638250299999996,
            21.630566024011667,
        ),
        "Spokane": (
        ...
        "Elko": ("Elko County, Nevada, United States", 41.1958128, -115.3272864),
        "Albuquerque": (
        ...
        "Sioux Falls": (
            "Sioux Falls, Sioux Falls Township, Minnehaha County, South Dakota, United States",
            43.5476008,
            -96.7293629,
        ),
        "Shreveport": (
        ...
        "Columbia": ("South Carolina, US", 4.099917, -72.9088133),
        "Indianapolis": (
        ...
        "Richmond": ("Richmond, Virginia, United States", 37.5385087, -77.43428),
        "Charleston": (
            "Charleston, Charleston County, South Carolina, United States",
            32.7884363,
            -79.9399309,
        ),
        "Baltimore": ("Baltimore, Maryland, United States", 39.2908816, -76.610759),
        "Syracuse": (
            "City of Syracuse, Onondaga County, New York, United States",
            43.0481221,
            -76.1474244,
        ),
        "Augusta": (
        ...
        "Sitka": ("Sitka, Alaska, 99835, United States", 57.4086082, -135.4596206),
        "Helena": (
        ...
        "Boise": ("Boise, Ada County, Idaho, United States", 43.6166163, -116.200886),
        "San Jose": (
        ...
        "Santa Fe": ("Santa Fe, Argentina", -30.3154739, -61.1645076),
        "Portland": (
        ...
        "Pierre": (
            "Pierre, Hughes County, South Dakota, 57501, United States",
            44.3683644,
            -100.351136,
        ),
        "San Antonio": (
        ...
        "Memphis": (
            "Memphis, Shelby County, West Tennessee, Tennessee, United States",
            35.1460249,
            -90.0517638,
        ),
        "Norfolk": ("Norfolk, England, United Kingdom", 52.666667, 1.0),
        "Milwaukee": (
        ...
        "Cold Bay": (
            "Cold Bay, Aleutians East Borough, Alaska, 99571, United States",
            55.2071632,
            -162.7146533,
        ),
        "Bethel": (
            "Bethel, Unorganized Borough, Alaska, 99559, United States",
            60.7922222,
            -161.755833,
        ),
        "Barrow": (
            "Barrow, Westmorland and Furness, England, United Kingdom",
            54.1007661,
            -3.2087523941640113,
        ),
        "Nome": (
            "Nome, Unorganized Borough, Alaska, United States",
            64.4975098,
            -165.4061701,
        ),
        "Valdez": (
            "Valdez, Unorganized Borough, Alaska, 99686, United States",
            61.1299396,
            -146.349363,
        ),
        "Juneau": ("Juneau, Alaska, United States", 58.3019496, -134.419734),
        "Fairbanks": (
        ...
        "Prudhoe Bay": (
            "Prudhoe Bay, Alaska, 99734, United States",
            70.326677,
            -148.94325269685487,
        ),
        "Minneapolis": (
        ...
        "St. Louis": ("Saint Louis, Missouri, United States", 38.6280278, -90.1910154),
        "New Orleans": (
        ...
        "Anchorage": ("Anchorage, Alaska, United States", 61.2163129, -149.894852),
        "San Francisco": (
            "San Francisco, California, United States",
            37.7792588,
            -122.4193286,
        ),
        "Denver": ("Denver, Colorado, United States", 39.7392364, -104.984862),
        "Houston": (
...
def add_location_to_raw():
    ...
    for location in tqdm(locations):
        ...
        if country == "United States":
            print(
                f"{location} -> {address} | {state} | {county} | {city} | {lat} | {long}"
            )
...
def micro_statistics():
    ...
    print(
        df.columns
    )  # 'City', 'Claim', 'Tweet', 'Latitude', 'Longitude', 'User', 'Timestamp', 'Stance', 'Category', 'State', 'Verdict']
    # print(election_df.columns)

    # print the number of unique verdict
    print(
        df["Verdict"].unique()
    )  # ['FALSE' 'barely-true' 'pants-fire' 'TRUE' 'half-true' 'mostly-true' 'full-flop' 'half-flip']
...
def macro_statistics():
    ...
    hatches = [
        ...
        "|||",
        "\\\\",
    ]  # Add hatches for shading
    plt.figure(
        figsize=(27, 14), dpi=250
    )  # Increase the resolution by setting the figure size and dpi
    ...
    plt.xlabel("Stance Distribution")
    plt.ylabel("Count")
    ...
    # Add legend
    legend_elements = [
        Patch(facecolor="green", edgecolor="black", label="$\oplus$"),
        Patch(facecolor="orange", edgecolor="black", label="$\odot$"),
        Patch(facecolor="red", edgecolor="black", label="$\ominus$"),
...
def classification():
    ...
    stance_labels = [
        ...
        "neutral",
        "negative",
    ]  # mapping: $\oplus$, $\odot$, $\ominus$

    # Ground truth labels (based on claim type)
    true_labels = ["positive"] * 11646 + ["neutral"] * 10897 + ["negative"] * 113497
    ...
    pred_labels = (
        ...
        + ["positive"] * 5094
        + ["neutral"] * 1350
        + ["negative"] * 4453  # Uncertain
        + ["positive"] * 64643
        + ["neutral"] * 9677
        + ["negative"] * 39177  # Misinformation
    )
    ...
    print(
        classification_report(
            true_labels, pred_labels, target_names=claim_types, digits=3
        )
    )
